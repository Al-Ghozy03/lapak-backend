...
async function getNotif(req, res) {
  try {
    const data = await sequelize.query(
      `select notif.id as notif_id,notif.from,notif.to,notif.message,notif.is_read,users.name,users.photo_profile,notif.created_at from notifications as notif join users on notif.from = users.id`,
      {
